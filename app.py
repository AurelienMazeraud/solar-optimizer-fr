...
with st.expander("Comment fonctionne ce simulateur ?"):
    st.markdown(
        "Cinq etapes : **1)** choisis ton profil (nouveau projet ou "
        "installation existante), **2)** situe ta maison sur la carte "
        "(si nouveau projet), **3)** decris ton installation (panneaux ou "
        "donnees reelles), **4)** decris ta consommation et ton "
        "financement, **5)** lis les resultats (energie et argent). Chaque "
        "section a son propre encadre d'explication -- pas besoin de "
        "connaissances techniques en amont."
    )
...
st.caption(
    ...
    "entierement facultatif, independant de la simulation ci-dessus."
)
...
with st.form("contact_form", clear_on_submit=True):
    ...
    contact_submitted = st.form_submit_button("Envoyer mes coordonnees", use_container_width=True)
